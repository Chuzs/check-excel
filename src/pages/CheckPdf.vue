<template>
  <div>
    <div class="container">
      <a-form>
        <a-row type="flex" justify="space-between">
          <a-form-item v-bind="validateInfos.totalDataFileList" name="totalDataFileList" label="预报关" >
            <a-upload
    ...
    <a-collapse v-model:activeKey="activeKey">
      <a-collapse-panel key="1" :header="'单个数据错误条数：'+failedData.length+'，正确条数：' + successData.length">
        <a-table rowKey="size" :columns="dataColumns" :data-source="[...toRaw(failedData), ...toRaw(successData)]" bordered size="small" >
      ...
      <a-collapse-panel key="2" :header="'每页汇总数据错误条数：'+failedPages.length+'，正确条数：' + successPages.length">
         <a-table rowKey="size" :columns="pageColumns" :data-source="[...toRaw(failedPages), ...toRaw(successPages)]" bordered size="small" >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'status'">
              <span>
                <a-tag :color="record.status === 1 ? 'volcano' : 'green'">
      ...
      <a-collapse-panel key="3" :header="'发票船号日期：' + (orderInfoSuccess? '正确':'错误')">
        <a-table rowKey="size" :columns="orderColumns" :data-source="[{...toRaw(totalOrderInfo),type:'预报关'}, {...toRaw(uncheckedOrderInfo),type:'pdf'}]" bordered size="small" >
          <template #bodyCell="{ column, text , record }">
            <template v-if="column.key === 'invDate'">
              <span :style="{color:record?.errList?.includes(column.key)?'red':'black'}">
            ...
            <template v-if="['invNo','vessel','country','destination'].includes(column.key)">
              <span :style="{color:record?.errList?.includes(column.key)?'red':'black'}">
               {{ text}}
              </span>
            </template>
  ...
  for (let i = 0; i < strArr.length; i++) {
    ...
    if (strArr[i].indexOf('Total G.W.') > -1) {
      ...
      if (numList.length === pdfDocument._pdfInfo.numPages - 1) {
        pageInfo.totalFob = parseFloat(strArr[i - 4].split(' ')[1].replace(',', ''))
      } else {
...
const getDraftPageData = async (pdfDocument, page) => {
  ...
  for (let i = 0; i < strArr.length; i++) {
    ...
    if (page === 1 && strArr[i].trim() === 'PASSENGER CAR RADIAL') {
      ...
      tmp.quantity = tmp.quantity === parseInt(strArr[i - 2].split(' ')[0]) ? tmp.quantity : tmp.quantity + '_' + parseInt(strArr[i - 2].split(' ')[0])
      ...
      tmp.quantity = tmp.quantity === parseInt(strArr[i + 11].split(' ')[0]) ? tmp.quantity : tmp.quantity + '_' + parseInt(strArr[i + 11].split(' ')[0])
    ...
    if (page === 1 && strArr[i].indexOf('NW =') > -1) {
      ...
      pageInfo.totalFob = parseFloat(strArr[i - 1].split(' ')[1].replace(',', ''))
      pageInfo.totalQty = parseInt(strArr[i + 1].split(' ')[2].replace(',', ''))
      pageInfo.totalGw = parseFloat(strArr[i + 2].split(' ')[2].replace(',', ''))
    } else if (strArr[i].indexOf('GW =') > -1) {
      pageInfo.totalQty = parseInt(strArr[i - 1].split(' ')[2].replace(',', ''))
      pageInfo.totalGw = parseFloat(strArr[i].split(' ')[2].replace(',', ''))
    } else if (strArr[i].indexOf('NW =') > -1) {
      pageInfo.totalNw = parseFloat(strArr[i].split(' ')[2].replace(',', ''))
      pageInfo.totalFob = parseFloat(strArr[i - 2].split(' ')[1].replace(',', ''))
    }
  }
  console.log(strArr.join('--'))
  numList.push(uncheckedData.length)
...
const getYifanPageData = async (pdfDocument, page) => {
  ...
  for (let i = 0; i < strArr.length; i++) {
    ...
    if (strArr[i].indexOf('0215564001841') > -1) {
      ...
      uncheckedOrderInfo.vesselDate = moment(strArr[i + 5], 'DD-MM-YYYY').subtract(543, 'years')
      uncheckedOrderInfo.country = strArr[i + 13].replace(' ', '') + strArr[i + 11]
      uncheckedOrderInfo.destination = strArr[i + 14].replace(' ', '') + strArr[i + 12]
    }
    if (strArr[i].trim() === 'MADE IN THAILAND') {
    ...
    if (strArr[i].indexOf('Total G.W.') > -1) {
      pageInfo.totalNw = parseFloat(strArr[i - 1].split(' ')[2].replace(',', ''))
      pageInfo.totalFob = parseFloat(strArr[i + 3].split(' ')[1].replace(',', ''))
      pageInfo.totalQty = parseInt(strArr[i + 1].split(' ')[0].replace(',', '')) === parseInt(strArr[i + 2].split(' ')[2].replace(',', '')) ? parseInt(strArr[i + 1].split(' ')[0].replace(',', '')) : parseInt(strArr[i + 1].split(' ')[0].replace(',', '')) + parseInt(strArr[i + 2].split(' ')[2].replace(',', ''))
      pageInfo.totalGw = parseFloat(strArr[i].split(' ')[2].replace(',', ''))
    }
  }
  console.log(strArr.join('--'))
